...
class RoutingExperiment
{
  ...
  private:
    ...
    uint64_t previousTcpBytes{0};  // Contatore per i byte TCP precedenti
    ...
    std::string m_movementSpeed{"NORMAL"};  // Nuovo flag per la velocità di movimento
    double m_txp{0.1};
...
RoutingExperiment::CheckThroughput()
{
    ...
    {
        uint64_t currentTcpBytes = 0;
        
        // Calcolo dei byte totali attuali ricevuti da tutti i sink
        for (auto& sink : sinks)
        {
            currentTcpBytes += sink->GetTotalRx();
        }
        
        // Calcolo dei byte ricevuti nell'intervallo corrente
        uint64_t intervalBytes = currentTcpBytes - previousTcpBytes;
        previousTcpBytes = currentTcpBytes;
        
        // Throughput in kbps nell'intervallo corrente
        kbs = (intervalBytes * 8.0) / 1000;
        
        // Pacchetti nell'intervallo (assumendo pacchetti da 1024 byte come suggerito
        // dal codice che imposta TcpSocket::SegmentSize)
        totalPackets = intervalBytes / 1024;
        if (intervalBytes % 1024 > 0 && intervalBytes > 0) {
            totalPackets++; // Aggiunge un pacchetto parziale se c'è un resto
        }
    ...
    out << Simulator::Now().GetSeconds() << ","
        ...
        << m_txp << ","
        << m_trafficType << ","
        << m_movementSpeed << std::endl;  // Aggiungiamo la velocità di movimento all'output
    out.close();
...
RoutingExperiment::CommandSetup(int argc, char** argv)
{
    ...
    cmd.AddValue("movement", "Movement speed (NORMAL/HIGH)", m_movementSpeed);  // Nuovo parametro
    ...
    std::vector<std::string> allowedMovementSpeeds{"NORMAL", "HIGH"};  // Velocità di movimento consentite
    ...
    // Verifica del parametro movement
    if (std::find(allowedMovementSpeeds.begin(), allowedMovementSpeeds.end(), m_movementSpeed) == allowedMovementSpeeds.end())
    {
        NS_FATAL_ERROR("Invalid movement speed (use NORMAL or HIGH):" << m_movementSpeed);
    }
...
RoutingExperiment::Run()
{
    ...
    out << "SimulationSecond,"
        ...
        << "TransmissionPower,"
        << "TrafficType,"
        << "MovementSpeed" << std::endl;  // Aggiungiamo MovementSpeed all'intestazione
    out.close();
    ...
    // Imposta la velocità dei nodi in base al parametro movement
    if (m_movementSpeed == "HIGH") {
        nodeSpeed = 40;  // Velocità alta
    } else {
        nodeSpeed = 20;  // Velocità normale
    }
